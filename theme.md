...
I originally use `remote_theme` but using `remote_theme` is not convenient when you need to override the original files: you cannot use git to view differences because `remote_theme` is not merged to your own repository.
...
## Category
I add categories for each blog post using jekyll's builtin category feature.
...
The moving theme uses bitter from google fonts, but I think hosting it locally is better, so I changed the theme to use local bitter font.
